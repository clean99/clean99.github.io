---
title: "Workspace v2 Tab System：把浏览器标签页带进工作空间"
...
case_study:
  ...
  impact:
    ...
    - "opened tabs 和 hot runtimes 分成两层：hot runtime pool 是有上限的 LRU，淘汰 runtime 只释放 DOM、iframe 和 JS 资源，不会删掉 tab。"
  links: []
---
...
图 0：一个通用工作台里的 tab system 演示。用户可以同时打开多个工作对象，像浏览器一样切回；只有当前 tab 拥有 URL、弹层、事件和前台 CPU。 generated by gpt-image-2.
...
图 A1：从产品目标到工程约束。它把“用户想要的浏览器级体验”拆成具体技术问题，也把失败后果写出来。 generated by gpt-image-2.
...
图 A2：Tab system 的最终分层。Isolation 和 Observability 是两件事：Isolation 是运行时边界，负责阻止 hidden runtime 改当前 tab 的 URL、弹层和事件；Observability 是横切能力，负责定位哪里慢、哪里被拦、哪里异常。 generated by gpt-image-2.
...
| iframe 沙箱 | `window.postMessage` envelope | 校验 origin 和 payload，再转成宿主 event bus 事件 |
...
图 A3：URL 和 tab 的双向同步。重点是：浏览器地址栏仍然是业务 URL，宿主在内部把它映射到 tab。 generated by gpt-image-2.
...
function activateTab(tab) {
  ...
  navigate(path, {
    ...
    // 这里把目标 tab id 写进 history.state，后续 popstate / hashchange / 沙箱 history
...
图 A3.5：Persistent tab state 的模块关系。React Query 让当前窗口先快起来，BFF 保存最终事实，BroadcastChannel 只通知其它窗口“事实变了”，其它窗口再自己从 BFF 拉最新 tab list。 generated by gpt-image-2.
...
图 A4：Runtime cache 的三层职责。Opened tabs 是持久化事实；Hot runtime pool 是有限热运行资源；Scoped view cache 是 Workstream runtime 内部的局部保活。Idle prewarm 是后台准备策略，不是无限后台加载。 generated by gpt-image-2.

| 层 | 回答的问题 | 生命周期 |
| --- | --- | --- |
| Opened Tabs | 这个 tab 是否存在 | BFF 持久化；最多 20 个 opened tabs；pin 不计入 cap |
| Hot Runtime Pool | 切回时能不能马上显示 | 最多 5 个 hot frames；保留 DOM 或 iframe 沙箱；LRU demote |
...
| Idle Prewarm | 用户点击前能不能先准备一部分 | 首屏后执行；native 更早，沙箱更晚；切换中暂停 |
...
class WarmPool {
  promote(tabId, location, runtimeKind) {
    ...
    // iframe 沙箱更重，所以有单独上限；全局 hot frame 也有总上限。
    // 淘汰只会卸载 runtime，不会删除 opened tab。
    evictOldestSandboxIfOverCap();
...
const candidate = {
  ...
  runtimeKind: tab.runtimeKind, // native / subapp / sandbox
};
...
![图 A5：沙箱接入与 tab 隔离](/img/workspace-v2-tab-system/sandbox-isolation.png)

图 A5：沙箱接入与 tab 隔离。沙箱框架负责加载子应用；Workspace host 在沙箱暴露的生命周期和 sandbox window 外围加 tab 归属上下文。 generated by gpt-image-2.

### 沙箱接入：从目标倒推能力

这一层的目标不是“用了哪些沙箱 API”。目标是更具体的四个不变量：

| 目标 | 不变量 |
| --- | --- |
| 复用沙箱加载能力 | 子应用仍然由沙箱框架的 `Sandbox` 组件加载，不重造一套 runtime |
...
| 用户能看到的问题 | 背后原因 / 沙箱约束 | 需要的边界 |
| --- | --- | --- |
| 用户切到 tab B 后，地址栏变成 tab A 的 URL，tab A 的弹窗盖到 B，或 tab A 开始刷新抢资源 | 沙箱框架默认解决“怎么加载子应用”，不知道这次 DOM / history / event 操作应该归属于哪个 Workspace tab | 宿主必须在沙箱外围补一层 tab 归属上下文，把 DOM、history、event 都挂到目标 tab |
...
| 用户没有切回 tab A，但当前地址栏被 tab A 的内部跳转改掉 | 沙箱自己维护 `RAW_HISTORY`，只 patch host history 不够 | patch `sandbox.raw.win.RAW_HISTORY`，按 tab target 校验 push / replace |
...
所以我们最后用到的沙箱能力，其实是被这些边界倒推出来的：

| 沙箱能力 | 满足哪个要求 | 接入方式 |
| --- | --- | --- |
| `Sandbox` | 继续复用沙箱框架的加载、entry、basename 和生命周期 | 宿主只包一层 tab owner，不替换沙箱 runtime |
...
| `sandbox.raw.win.RAW_HISTORY` | 接管沙箱真正使用的 history | 对 `pushState` / `replaceState` 做 tab target 校验，再决定是否同步 host history |
...
沙箱里有多层 history，只 patch 宿主 `window.history` 不够：

| History surface | 归属 | 风险 |
| --- | --- | --- |
| `window.history` | Workspace host | 直接改可见地址栏 |
| `iframeWin.history` | 沙箱内的 subapp | 默认不知道自己的 Workspace tab 是否 active |
| `iframeWin.RAW_HISTORY` | 沙箱 history plugin | 沙箱内部 push / replace 可能绕过 host history 检查 |
...
// 沙箱内部的 raw history 才是 sandbox 真正使用的 surface。
...
这样沙箱子应用仍然按原接口访问 `window.parent`，但拿到的是按 tab 裁剪后的对象。
...
当组件库调用 `document.body.appendChild(node)`，或者沙箱 runtime 往 scoped root 插入 body child 时，宿主先分类节点，再决定保留在哪里。
...
这层难点是 **用户看不到的 runtime 仍然会产生用户看得见的副作用**。如果不在沙箱 window 和 document API 边界加 owner，hidden tab 就不是“隐藏”，而是“后台仍然能改当前页面”。

## 第六层：Rendered Runtimes

### 问题场景

用户看到的是一个当前 tab，但异常会很明显：切到 B 后 A 的 iframe 还挡着，点击落到 A，URL 写到 A，或者指标说切换完成但页面不能操作。背后原因才是宿主同时管理 native Workstream、iframe 沙箱、MF subapp 三种 runtime，它们的生命周期和 location 来源都不一样。
...
| 同样是切 tab，有的页面状态保留，有的页面被重建 | native route、iframe 沙箱、MF iframe 的生命周期不同，没有统一容器收口 |
...
| 步骤 | 设置什么 | 用在哪里 |
| --- | --- | --- |
| 沙箱 runtime focus | `focusedRuntimeTabId` | scoped history 判断 sandbox history 写入是否允许同步到宿主 URL |
...
这样视觉 owner、沙箱 runtime owner、overlay owner、event owner、metric owner 是同一个 tab。

### 难点

这层难点是 **用户看见的 focused tab 必须和运行时 owner 一致**。如果视觉上切到了 A，但沙箱 scope 还认为 B 是 focused，history、overlay、event 都会错。
...
| 沙箱 | 每次切换 reload 最干净，但状态丢失、切换慢 | 保留沙箱 runtime，并在 host 边界加 scope |
