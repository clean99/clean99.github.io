...
case_study:
  ...
  impact:
    ...
    - "Host uploader work was removed from the pre-FMP path on 7/7 routes to 0/7, improving 6/7 main paths by about -1037ms on average."
  links: []
---
...
| Background Pressure | Hidden tabs, prewarm jobs, SDKs, WebSocket, analytics, and monitoring tasks may steal main-thread time while the user switches tabs. | long task, frame gap, foreground lease, local stress gate. Background work must yield to the foreground tab. |
...
The table below is a weekly field-report view. It shows scale and trend. It is not the same measurement as the strict preview profiles used later for individual changes.
...
| Sandbox entry fanout | Non-current sandbox entry competed with the current route. | affected routes `5` | `0` | average about `-366ms` vs previous accepted run | strict FMP loop; non-current sandbox entry moved after FMP |
...
| Background pressure | Background SDKs could become future tab-switch jank sources. | max long task present risk | max post-visible long task = 0 | guardrail, not preview-environment main win | local stress gate; Notification/WebSocket/analytics/monitoring moved into scheduler |

Two guardrails:

1. Strict preview FMP, strict tab switch, and the local stress gate are not combined into one total number.
...
E2E cases protect correctness while performance changes move work around. Mock-off integration connects through preview-environment routing to the real backend instead of mistaking mock data for preview evidence.
...
| --- | --- | --- | --- | --- | --- |
| Measurement contract | Old tab switch duration looked fast while users could still see an unclickable page. FMP experiments could also treat lower resource count as a false win. | Shell visible and interactive were mixed; local gate, preview FMP, and strict switch had different meanings. | FMP uses final first-screen marker; tab switch v3 measures input, visible, interactive, and post-visible blocking; the local stress gate is only a regression gate. | Later wins can point to blocker movement or reduced post-visible blocking. | Measurement repair itself is not counted as a performance win. |
| Critical-path reduction | Current page was not visible yet, but network and CPU were serving future paths. | Workstream list, host uploader, AIS, and non-current sandbox entry fanout entered pre-FMP. | Keep only current-route first-screen work before FMP; schedule low-priority SDK and non-current runtime work after first-screen ready. | `/workspace/api/workstream/list` moved out of pre-FMP on 6/7 root-subapp paths; host uploader `7/7 -> 0/7`; sandbox fanout affected routes `5 -> 0`. | Only defer work that is not part of current first screen; if FMP gets worse, revert. |
...
*Figure P2: Critical-path cleanup waterfall. This is a strict FMP-loop pattern chart. It explains how host uploader moved from 7/7 pre-FMP routes to 0/7, and non-current sandbox entry fanout moved from 5 affected routes to 0. The example route is illustrative; exact route deltas are covered by Scheduling and Official Forecast.*
...
| Defer non-current sandbox entry fanout | Sandbox app manifests and `static/js/entry.*` competed with the current route. | affected routes `5 -> 0` | average about `-366ms` vs previous accepted run. |
...
- the remaining FMP window is sandbox runtime, data, and render, not late code discovery.
...
// The iframe sandbox is heavier and needs a more conservative delay.
afterFirstScreenReady(() => {
...
The tradeoff is explicit: prewarming every runtime immediately is simple, but the iframe sandbox is heavier and can turn a background optimization into foreground pressure. The final policy warms native runtimes earlier and sandbox runtimes more carefully.
...
Problem: while the user switched to Supervisor, hidden/background Audit Workbench, xlsx, monitoring, and related work competed on the main thread. The solution is a foreground lease: while the foreground tab is switching or not settled, background tasks must yield.
```typescript
...
Problem: hidden tabs can still be alive, and SDKs such as Notification, WebSocket, analytics, monitoring, visit, storage health, and AIS can create future tab-switch long tasks if they run like a normal single-page app. The solution is a foreground-aware scheduler.
```typescript
...
| --- | --- | --- |
| MF preload | Re-check foreground state before and after `getEntries`. | Local stress gate: p95 visual about `31.2ms`, postVisibleBlocking about `7.8ms`, max long task = 0. |
| Notification SDK | Notification bundle about `657.2KB gzip`; re-check before/after import/open/render. | Local stress gate: postVisibleBlocking about `7.1ms`, max long task = 0. |
| WebSocket | Import, init, and register enter scheduler. | Local stress gate: visual `42.1ms -> 36.6ms`, about `-13.1%`. |
| Analytics | Flush queue in slices and yield to foreground. | Local stress gate: visual `31.5ms -> 29.5ms`, about `-6.3%`. |
| Monitoring / visit / storage health / AIS | after first screen, idle, foreground-aware. | Guardrail; not claimed as preview-environment main win. |
...
| Broad sandbox prewarm / hidden layout-visible | Hoped to finish sandbox work earlier | Easy to steal foreground CPU; unstable benefit; not used as the main policy. |
...
1. **Keep local gates and preview wins separate.** The local stress gate is a regression gate, not an online/preview win.
