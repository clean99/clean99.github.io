---
title: "Workspace v2 tab system: browser tabs inside a workspace"
date: 2026-05-18 16:30:00
tags: [Frontend, React, Software Engineering]
area: engineering
summary: "How we made workspace tabs keep state, survive refresh, share real URLs, and stop hidden runtimes from leaking side effects."
featured: true
...
case_study:
  ...
  impact:
    ...
    - "Opened tabs and hot runtimes are separate layers: the hot runtime pool is a bounded LRU, so evicting a runtime frees DOM, iframe, and JS resources without deleting the tab."
  links: []
---
...
| iframe sandbox | `window.postMessage` envelope | Validate origin and payload, then convert to a host event bus request. |
...
function activateTab(tab) {
  ...
  // Some subapps and the iframe sandbox will keep syncing URL in background
...
candidate = {
  id: tab.id,
  location: `tab's URL`,
  runtimeKind: native / subapp / sandbox,
};
...
![Figure A5: sandbox integration and tab isolation](/img/workspace-v2-tab-system/sandbox-isolation.png)

*Figure A5: Sandbox integration and tab isolation. The sandbox framework loads the subapp; the Workspace host adds tab ownership around the sandbox lifecycle and sandbox window boundaries. generated by gpt-image-2.*

### Sandbox Integration

| Category | Problem | Cause / Sandbox Constraint | Boundary Needed |
| --- | --- | --- | --- |
| Hidden tab updates the visible browser URL. | Sandbox runtimes can still call `window.history` or the sandbox's `RAW_HISTORY` while kept alive in the warm pool. | Add a host tab-ownership context around the sandbox runtime. |  |
...
| The first sandbox route write is missed, or early patching breaks sandbox startup. | The raw sandbox window is only reliable after the sandbox framework reports ready; patching too late misses the first route replace. | Register the runtime frame in onSandboxReady(), then reset the initial URL and install tab-scoped patches. |  |
| Switching back shows another tab's content, scroll state, or mounted DOM. | If sandbox content mounts into one global container, multiple hot tabs share the same DOM owner. | `getContainer()` must return the current `HotTabFrame` root, so each tab owns a stable DOM subtree. |  |
...
| Sandbox Capability | Requirement | Integration |
| --- | --- | --- |
| `Sandbox` | Reuse the framework's loading, entry, basename, and lifecycle. | The host wraps tab owner around the sandbox runtime; it does not rebuild the runtime. |
...
| `sandbox.raw.win.RAW_HISTORY` | Patch the history the sandbox actually uses. | Validate `pushState` / `replaceState` target before syncing host history. |

Integration order:
```javascript
function SandboxTabRuntime({ tabId, entry, initialUrl }) {
...
The sandbox architecture has a lot of `history`:

| History | Belongs to | Usage | Risk |
| --- | --- | --- | --- |
| window.history | Workspace main app | It will change the url directly | NA |
| iframeWin.history | iframe sandbox sub app history | Sub app code call `window.history` | By default it doesn't know whether workspace tab is active or not |
| iframeWin.RAW_HISTORY | Sandbox history plugin maintained history | The sandbox uses it to simulate/sync its internal router | If we don't inspect, sandbox-internal replace/push will ignore tab status |
...
or the sandbox runtime inserts body children into the scoped root, we classify the node before keeping it there.
```javascript
...
The user sees one current tab, but the host may keep native Workstream, iframe sandbox, and MF subapp runtimes alive. If these runtimes are treated as ordinary React components, users see direct failures:
...
| Some pages keep state while others remount. | Native routes, iframe sandbox, and MF iframe have different lifecycle models. |
...
| --- | --- | --- |
| Sandbox runtime focus | focusedRuntimeTabId | Scoped history checks whether a sandbox history write may update the host URL. |
...
| Sandbox | Reloading on every switch is clean, but it loses state and makes switching slow. | Keep the sandbox runtime warm, then scope host-facing APIs. |
