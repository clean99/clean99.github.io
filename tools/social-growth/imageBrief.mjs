...
export async function buildImageBrief(preflight, {
  ...
  return {
    ...
    commands: {
      ...
      ingestLatest: ingestLatestCommand(preflight.selected.id),
...
If the image was just generated by the built-in imagegen tool and saved under Codex's default generated-images directory, ingest the newest generated PNG directly:

\`\`\`bash
${brief.commands.ingestLatest}
\`\`\`
...
function ingestLatestCommand(id) {
  return `npm run social:ingest-imagegen -- --id ${id}`;
}
