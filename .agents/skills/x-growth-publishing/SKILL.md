...
- Use the installed open-source `writing-clearly-and-concisely` skill as a ruthless clarity pass before `humanizer-zh`: active voice, concrete nouns, positive claims, fewer needless words, and no puffed-up AI vocabulary. It is copied into `.agents/skills/` for this project so the X workflow can keep using it across sessions.
...
- Use the built-in engagement seed accounts in `tools/social-growth/engagement.mjs` for read-only Chinese X discovery. `@lxfater` and `@lidangzzz` are structure references only: find conversations and observable patterns, then write replies in Clean993's engineering voice. Do not imitate persona, insults, or audience-baiting.
...
- Use the installed open-source `x-twitter-growth` skill as an external X growth reference after Codex restart. Use it for profile audits, thread/reply strategy, competitor research prompts, and posting cadence sanity checks. Treat its tweet composer as a draft scaffold only; final Chinese copy still goes through `x-technical-sharing`, `humanizer-zh`, and the local quality gate.
- Use the installed open-source `social-media-analyzer` skill when reviewing exported X metrics or comparing post variants. Prefer its formulas for engagement rate, CTR, top-post ranking, and benchmark sanity checks, then write the resulting hypothesis back into the local ledger/recommendation flow instead of creating a separate metrics source.
- Use the project-installed open-source `writing-voice` and `social-media` skills as an extra de-AI writing pass after skill reload. Their strongest rules for this project are: one concrete idea per X post, no hype language, no template sections, no generic CTA, no hashtags by default, and no raw blog URL in the first short post.
- Use the project-installed open-source `good-prose` skill as a final prose audit when a draft still feels generated. Its relevant guardrails are: point first, concrete verbs, no significance inflation, no negative parallelism, no forced rule-of-three, and no generic optimistic close.
- Use the installed open-source `marketing-social` skill as a broad platform strategy reference for content pillars, repurposing, calendar shape, and reverse-engineering ideas. Do not use its scheduling or generic platform automation advice directly; convert useful ideas into local queue experiments and keep all public X actions behind the checklist/Chrome confirmation boundary.
...
- Use the installed open-source `x-collect`, `x-filter`, `x-create`, and `x-publish` skills from `kangarooking/x-skills` as an external Chinese X workflow reference. The useful parts are 4-round source collection, weighted topic scoring, A/B draft critique, stateful liked/rejected topic memory, and draft-only browser publishing discipline. Keep this project's `queue.json`, package ids, image paths, ledger, public-action checklist, and Chrome action-time confirmation as the source of truth; `x-publish` may prepare or save drafts only and must never be treated as permission to publish, upload media, reply, like, repost, follow, edit profile, or pin content.
...
- For X Article packages, use the fixed writing review order: `x-article-editor` for article structure, `writing-voice` + `social-media` for platform-native tone, `good-prose` for AI-artifact cleanup, then `humanizer-zh` for Chinese final polish. This chain is advisory only and never authorizes publishing, media upload, replies, likes, reposts, follows, profile edits, or pinning.
...
- Use the installed open-source `skill-judge` skill when reviewing or expanding project skills. Treat it as a design-quality reviewer for `SKILL.md` files, not as a publishing controller.
...
When the CDP publishing profile is logged out but the user-facing normal Chrome session is visibly logged into `@Clean993`, record `userBrowserAccount`, `userBrowserLoginState`, `userBrowserCurrentUrl`, and `userBrowserTitle` in the probe. The readiness status should become `ready_via_user_chrome_confirmation`, which means use the manual publish kit in normal Chrome and still stop before media upload and final publish clicks.
