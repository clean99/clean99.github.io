...
If the project-level `social-media` and `writing-voice` skills are available, run them as a platform-native tone check: one concrete idea per short post, no hype, no template sections, no generic CTA, no hashtags by default, and no raw blog URL in the first post.
...
If the project-level `good-prose` skill is available, use it as an AI-artifact audit before Chinese polish: remove significance inflation, negative parallelism, forced three-part lists, vague attribution, and generic optimistic endings.
...
If the project-level `x-article-editor` skill is available, use it only to audit X Article title, hook, skimmability, natural voice, proof, image placement, and close. Its review does not replace this skill's Chinese technical constraints or output contract.
