...
function stripStorybookClientDirectives(): Plugin {
  return {
    name: "liquid-glass-docs-client-directives",
    enforce: "pre",
    transform(code, id) {
      const isModule = /\.[cm]?[jt]sx?(?:\?.*)?$/.test(id);
      const isLiquidGlassSource =
        id.includes("/packages/liquid-glass/src/") || id.includes("/packages/liquid-glass/dist/");
      const isKnownClientDependency = id.includes("/react-resizable-panels/");

      if (!isModule || (!isLiquidGlassSource && !isKnownClientDependency)) {
        return null;
      }
...
const config: StorybookConfig = {
  stories: [
    ...
    "../../../packages/liquid-glass/stories/**/*.stories.@(ts|tsx)"
  ],
