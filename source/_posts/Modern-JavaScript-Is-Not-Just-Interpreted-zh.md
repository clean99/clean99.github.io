...
```txt
对象起始地址 + age 的 offset = age 的地址
```

然后直接读。

JS 对象如果没有这种优化，访问 `user.age` 会更像：
...
这就接近 `Map.get("age")`，成本明显更高。

Hidden Class 做的事情，可以简化理解成：
...
```txt
user1.storage = ["Tom", 18]
user2.storage = ["Jerry", 20]
```
...
```txt
HiddenClass A 告诉我 age 在 slot1
  ↓
读取 storage[1]
```

这就是为什么它接近数组访问。数组访问 `arr[1]` 本来就是按索引定位；Hidden Class 让对象属性访问也能先把属性名映射成稳定 slot，再按 slot 取值。
...
属性顺序变了，V8 可能需要另一个 Hidden Class：
...
如果一个热路径里一会儿来 A，一会儿来 B，一会儿又来更多 shape，Inline Cache 和 TurboFan 都会更难优化。
...
这个对象可能是 HiddenClass B，而且根本没有 `name`。Inline Cache 就要记录更多情况：

```txt
if HiddenClass A -> slot0
if HiddenClass B -> not found
```

这叫 polymorphic IC，多态缓存。还能优化，但已经比单态复杂。
...
它就可能变成 megamorphic IC。这个时候 V8 基本会说：这个地方太乱了，别猜了，走更通用的查找逻辑吧。
