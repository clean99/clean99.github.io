...
Tab system 引入后，工作台不再只有一个前台页面。用户可以同时打开多个 workstream、子应用和对象页，系统背后也会同时存在更多 route、runtime、iframe、SDK 和后台任务。性能问题因此从单页 FMP，变成了跨 Workspace shell、多个子应用和多个常驻 runtime 的资源归属问题。
...
## Measurement & Gates：验证 Harness 和 E2E 用例
...
这套机制比单点优化更重要，因为 Workspace 只会接入更多子应用、更多 SDK、更多 runtime。没有路径分类和资源仲裁，tab 越多，系统越容易失控。
